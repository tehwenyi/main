...
[NOTE]
ePiggy launches with some sample data preloaded. To start afresh, use the `clear` command to reset everything.
...
The INDEX refers to the number in the displayed Allowance &
Expenses List which is next to the name of the allowance. +
*Alias:* `da` +
...
Figure of example 1: Completed report.
