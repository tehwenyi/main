...
This command can only be used if there are no budgets set. The setBudget command is case sensitive. +
Format: `setBudget [$/MONEY] [t/TIME_PERIOD_IN_DAYS]`

Examples:

* `setBudget $/500.00 t/7 d/03/02/2019` +
Sets a total budget of $500.00 for each week starting from 03/02/2019.

* `setBudget $/10000.00 t/15 d/01/01/2000` +
Sets a total budget of $10000.00 every 15 days starting from 01/01/2000.
...
Edits the current budget. +
The editBudget command is case sensitive. +
Format: `editBudget [$/MONEY_WITH_TWO_DECIMAL_PLACES] [t/TIME_PERIOD_IN_DAYS]`
