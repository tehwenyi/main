...
.  Install Java (version 9 or later) on your computer by going here and downloading the Java SE Runtime Environment for
   your operating system.
.  Download the latest *`ePiggy.jar`* file from here
.  Copy the file to the folder which you want to use as the home folder for your Expense Tracker.
.  Double-click the *`ePiggy.jar`* file to start the app. The GUI will appear and look like this:
+
