...
public class DeleteAllowanceCommandTest {
    ...
    public void execute_validIndexIsAllowance_deleteSuccess() {
        Allowance allowanceToDelete = (Allowance) model.getFilteredExpenseList().get(INDEX_FIRST_ALLOWANCE.getZeroBased());
        DeleteAllowanceCommand deleteAllowanceCommand = new DeleteAllowanceCommand(INDEX_FIRST_ALLOWANCE);

        String expectedMessage = String.format(DeleteAllowanceCommand.MESSAGE_DELETE_ALLOWANCE_SUCCESS, allowanceToDelete);
    ...
    public void execute_validIndexIsExpense_throwsCommandException() {
        Model tempModel = new ModelManager(seedu.address.testutil.epiggy.TypicalExpenses.getTypicalEPiggy(), new UserPrefs());
        Expense expenseToDelete = (Expense) tempModel.getFilteredExpenseList().get(INDEX_FIRST_EXPENSE.getZeroBased());
        DeleteAllowanceCommand deleteAllowanceCommand = new DeleteAllowanceCommand(INDEX_FIRST_EXPENSE);

        String expectedMessage = String.format(DeleteAllowanceCommand.MESSAGE_ITEM_NOT_ALLOWANCE, expenseToDelete);
    ...
    public void execute_invalidIndex_throwsCommandException() {
        ...
        assertCommandFailure(deleteAllowanceCommand, model, commandHistory, DeleteExpenseCommand.MESSAGE_INDEX_OUT_OF_BOUNDS);
    }
}
