...
import seedu.address.testutil.epiggy.TypicalGoal;
...
public class JsonAdaptedGoalTest {
    ...
    private static final String VALID_NAME = TypicalGoal.IPHONE.getName().toString();
    private static final String VALID_COST = String.valueOf(TypicalGoal.IPHONE.getAmount().getAmount());

    @Test
    public void toModelType_validGoal_returnsGoal() throws Exception {
        JsonAdaptedGoal goal = new JsonAdaptedGoal(TypicalGoal.IPHONE);
        Assert.assertEquals(TypicalGoal.IPHONE, goal.toModelType());
    }
